...
def host_imagecache_create(context, values):
    ...
    host:string
    cache_id:string
    survival_value:integer
    size:integer
    """
    datetime_keys = ('created_at', 'deleted_at', 'updated_at')
    convert_objects_related_datetimes(values, *datetime_keys)

    host_imagecache_ref= models.HostImageCache()
    host_imagecache_ref.update(values)
    host_imagecache_ref.save()
    return host_imagecache_ref

def host_imagecache_delete(context, host, cache_id):
    #FIXME:should use soft_delete ?
    return model_query(context, models.HostImageCache).\
            filter_by(host=host).\
            filter_by(cache_id=cache_id).\
            delete()

def host_imagecache_update(context, values):
    #NOTE:if the imagecache doesn't exsit then create it
    """
    values:
    host:string
    cache_id:string
    ...
    """
    host=values['host']
    cache_id=values['cache_id']

    session = get_session()
    with session.begin():
        #NOTE:because we use delete() to delete one record ,so there is only one by query
        host_imagecache_ref=model_query(context, models.HostImageCache, session=session).\
                                    filter_by(host=host).\
                                    filter_by(cache_id=cache_id)

        #TODO:should we use update,deleted_at , updated_at, created_at
        """
        values['updated_at'] = timeutils.utcnow()
        datetime_keys = ('created_at', 'deleted_at', 'updated_at')
        convert_objects_related_datetimes(values, *datetime_keys)
        """
        if host_imagecache_ref.all():
            host_imagecache_ref.update(values)
        else:
            host_imagecache_create(context, values)

    return host_imagecache_ref

def host_imagecache_get_all_by_host(context, host):
    return model_query(context, models.HostImageCache, read_deleted="no").\
                filter_by(host=host).\
                all()

def host_imagecache_get(context, host, cache_id):
    return model_query(context, models.HostImageCache, read_deleted='no').\
                filter_by(host=host).\
                filter_by(cache_id=cache_id).\
                first()
